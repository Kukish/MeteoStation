...
void DS3231::adjust(const char* date, const char* time) {
	uint8_t yy, m, d, hh, mm, ss;
	...
	mm = conv2d(time + 3);
	ss = conv2d(time + 6);
	_writeRegister(REG_HOUR, _encode(hh));
	_writeRegister(REG_MIN, _encode(mm));
	_writeRegister(REG_SEC, _encode(ss));
	_writeRegister(REG_YEAR, _encode(yy));
	_writeRegister(REG_MON, _encode(m));
	_writeRegister(REG_DATE, _encode(d));
}
...
void DS3231::setMinute(uint8_t min)
{
	if ((min>=0) && (min<60))
	{
		_writeRegister(REG_MIN, _encode(min));
...
char *DS3231::getTimeStr(uint8_t format, char divider)
{
